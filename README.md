...
| urlscan.io free tier scan speed | The free tier submission queue is shared across all public users. After a scan is submitted, the pipeline waits 10 seconds before polling and retries for up to 70 seconds. This makes each scan take approximately 20 to 25 seconds. Paid tiers offer faster processing and a dedicated queue, but are outside the project budget. |
| Cloaking by malicious sites | Some malicious sites detect known scanner IP ranges used by urlscan.io and Google Safe Browsing crawlers, and serve clean content to them while serving malicious content to real users. When this happens, both external APIs report the site as safe and the scan result will be SAFE despite the site being harmful. Confirmed malicious domains should be manually added to the URL Registry as a mitigation. |
| RDAP coverage gaps | Not all domain registrars support the RDAP protocol. Some country-code TLDs and older registrars only expose WHOIS, which rdap.org cannot always translate. When RDAP data is unavailable, the domain age field will be blank in the scan result. |
| CDN and proxy geolocation | Server location is derived from the IP address returned by urlscan.io. Sites behind CDN providers such as Cloudflare or Akamai will always show the CDN data centre country rather than the actual hosting country, making the server location field unreliable for CDN-proxied sites. |
| urlscan.io public scan visibility | The free tier requires all submitted URLs to be publicly visible on urlscan.io. Any URL scanned through LinksLens appears in urlscan.io public search results, which is a privacy consideration for users scanning sensitive or internal URLs. |
| Zero-day phishing and malware sites | Newly registered malicious domains not yet indexed by Google Safe Browsing or the urlscan.io community threat feed will not be flagged. Detection accuracy depends entirely on how quickly those external services update their databases after a malicious site goes live. |
